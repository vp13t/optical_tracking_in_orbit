...
def run(args):
    ...
    estimators = [pdaf.PDAF(x0_est, tune.P0, tune.Q, tune.R, tune.PG, tune.PD, tune.Lambda)]
    estimator = gmf_pdaf.GMF(estimators, max_filter_count=8)
    ...
    for i in range(xhist_sat1.shape[1]-1):
        ...
        h_null = mt_ni.gen_h_rel(next_state, theta, estimated_target_obj)
        xhat, Pk = estimator.measurement(meas, h, tune.R, h_null, tune.R_ni)
