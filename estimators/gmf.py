...
class GMF:
    ...
    def measurement(self, y, h, R, reweight_only=False):
        innovs = list(self.executor.map(lambda Fi: Fi.innov(y, h, R), self.filters))
        if not reweight_only:
            def meas_in_place(Fi):
                Fi.measurement(y, h, R)
                return Fi
            self.filters = list(self.executor.map(meas_in_place, self.filters))

        weight_updates = np.array([multivariate_normal(yhat, innov_cov).pdf(y) for yhat, innov_cov in innovs])

        weights = self.weights * weight_updates
